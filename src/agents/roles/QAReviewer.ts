...
3. **장면 수**: 4~8개 장면이 있는지
4. **프레임 범위**: 각 장면의 durationInFrames가 60~150 사이인지
5. **색상 유효성**: hex 코드가 올바른 형식인지
6. **가독성**: 텍스트와 배경의 대비가 충분한지
7. **일관성**: 전체 컬러 스킴과 개별 장면 색상이 조화로운지
8. **콘텐츠 품질**: 각 장면의 content가 구체적이고 의미 있는지 (제네릭 문구 X), 리서치 결과의 실제 정보가 반영되어 있는지
